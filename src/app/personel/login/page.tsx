...
export default function PersonelLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        // Kullanıcının gerçekten çalışan olup olmadığını kontrol et
        const docRef = doc(db, 'employees', user.uid)
        const docSnap = await getDoc(docRef)

        if (docSnap.exists()) {
          // Çalışan ise personel paneline yönlendir
          router.push('/personel')
        } else {
          // Çalışan değilse (Patron ise) hata ver ve çıkış yaptır
          setError('Bu bir admin hesabıdır. Lütfen admin giriş sayfasını kullanın.')
          await auth.signOut()
          setLoading(false)
        }
      }
    ...
    <div style={{
      ...
      <div className="card" style={{ padding: '40px', width: '100%', maxWidth: '400px', background: 'white' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '2rem', color: 'var(--charcoal)' }}>Personel Girişi</h1>
          <p style={{ color: 'var(--muted)', fontSize: '14px' }}>Este Tuzla Personel Paneli</p>
        </div>
